...
#define MAX_STR_VALUE  32 // the max string value length
...
int ini_get_int(const char *file,const char* sect, const char *key, int defa)
{
	int ret;
...
int ini_get_string(const char *file, const char* sect, const char *key, char *strdefa, char * buf)
{
	char * tempbuf =(char*)  malloc(32);
	int lendefa,retlen;
	...
	if(find_string(tempbuf, sect, key, file))
	{
		...
		retlen = lendefa < MAX_STR_VALUE ? lendefa:MAX_STR_VALUE;
		strncpy(buf,strdefa,MAX_STR_VALUE);//something wrong in ini file ,not section ,not key and so on
	}
	else
	{
		retlen = MAX_STR_VALUE;
		strncpy(buf,tempbuf,MAX_STR_VALUE);
	}
	*(buf+retlen) = '\0';
	free(tempbuf);
...
void CIniMgr::GetValue(const char *file, const char* sect, const char *key, int& nValue, int defVal)
{
	nValue = ini_get_int(file, sect, key,defVal);
}

void CIniMgr::GetString(const char *file, const char* sect, const char *key, char* pString, char *strdef)
{
	ini_get_string(file, sect, key, strdef, pString);
}